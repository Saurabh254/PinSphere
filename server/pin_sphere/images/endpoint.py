...
async def upload_image(
    ...
    try:
        ...
        image = await service.save_image(current_user, file.file, username, session)
        return image
...
async def get_image(
    ...
    """
    image = await service.get_image(current_user, image_id, session)
    if not image:
...
async def delete_image(
    ...
    """
    success = await service.delete_image(current_user, image_id, session)
    if not success:
        raise HTTPException(status_code=404, detail="Image not found")
