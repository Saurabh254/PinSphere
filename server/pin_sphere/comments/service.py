...
async def create_comment(
    user: User, content_id: UUID, text: str, session: AsyncSession
):
...
async def create_reply(user: User, comment_id: UUID, text: str, session: AsyncSession):
...
async def delete_comment(user: User, comment_id: UUID, session: AsyncSession):
    result = await session.execute(select(Comment).where(Comment.id == comment_id))
...
async def toggle_like_comment(
    user: User, comment_id: UUID, like: bool, session: AsyncSession
):
    """
