...
const RAND_HEIGHTS = [70, 34, 40, 108];

const _CardShimmer = () => {
  const randInt = RAND_HEIGHTS[getRandomInteger(0, 3)];

  return (
    <div className="break-inside-avoid mb-4 p-2 border-2 border-gray-700  rounded-xl flex flex-col  opacity-90 animate-pulse ">
      <div className="w-full flex gap-2 ">
        <div className="w-6 h-6 rounded-full bg-gray-700"></div>
        <div className="flex-1 h-6 rounded-3xl bg-gray-700"></div>
      </div>
      <div
        className={`bg-gray-700 w-full mt-2 rounded-xl`}
        style={{ height: `${randInt * 2}px` }}
      ></div>
      <div className="flex opacity-40 items-center justify-left mt-1  text-sm">
      ...
      <div className="w-full h-4 bg-gray-700 rounded-xl"></div>
    </div>
  );
};
...
const CardsShimmer = () => {
  return (
    <div className="columns-2 overflow-hidden md:columns-3 lg:columns-4 gap-4 p-4">
      {Array.from({ length: 16 }).map((_, idx) => (
        <_CardShimmer key={idx} />
      ))}
    </div>
  );
};
